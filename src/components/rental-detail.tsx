...
import { dateTime, money, LABEL, isMeasured, diagnosedLabel, priceCaption, balanceLabel, isOpenEndedPickup, pickupForecastLabel, pickupOverdue, PRECISION_LABEL, customerPlaceLabel } from '../shared/format';
import { received } from '../shared/reports';
import { transitionForm, scheduleForm, paymentForm, voidForm, schedulePickupForm, regularizePickupForm } from './forms';
...
export function RentalDetail({ id, onClose }: {
    ...
    const bin = s.containers.find(c => c.id === r.containerId), customer = s.customers.find(c => c.id === r.customerId), site = s.customerSites.find(item => item.id === r.siteId), jobs = s.jobs.filter(j => j.rentalId === id), events = s.events.filter(e => e.rentalId === id), payments = s.payments.filter(p => p.rentalId === id), driver = s.user.role === 'DRIVER', paid = received(s, id);
 ...
 {!driver && isOpenEndedPickup(r) && ['RESERVED', 'DELIVERING', 'ACTIVE'].includes(r.status) && <Button small icon="calendar" onClick={() => openForm(schedulePickupForm(s, run, r))}>Programar retirada</Button>}
 {!driver && ['ACTIVE', 'COLLECTING'].includes(r.status) && !r.pickedUpAt && <Button small onClick={() => openForm(regularizePickupForm(s, run, r))}>Regularizar coleta já ocorrida</Button>}
 ...
 <section><h3>Local e responsáveis</h3><p className="address-line">{r.address}<br />{r.neighborhood} · {r.city}</p><div className="detail-grid"><div><small>Contratante</small><strong>{customerPlaceLabel(customer, site)}</strong><span>{customer?.contact}</span></div><div><small>Responsável no local</small><strong>{r.siteContact}</strong><a href={`tel:+55${r.sitePhone}`}><Icon name="phone" size={14}/> {r.sitePhone}</a></div></div><p className="muted">{PRECISION_LABEL[r.locationPrecision ?? 'PENDING']}{r.locationPrecision === 'APPROXIMATE' ? ' — o ponto no mapa não é o endereço exato.' : r.locationPrecision === 'PENDING' ? ' — o endereço está cadastrado, a posição no mapa ainda não foi confirmada.' : ''}</p><a className="text-button" target="_blank" rel="noopener noreferrer" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(r.locationPrecision === 'CONFIRMED' && r.latitude !== null && r.longitude !== null ? `${r.latitude},${r.longitude}` : `${r.address}, ${r.neighborhood}, ${r.city}`)}`}><Icon name="external" size={14}/> Abrir local no Google Maps</a></section>
 <section><h3>Assinaturas digitais</h3><div className="detail-grid">{deliverySign && deliveryDriverSign ? <div><small>Entrega</small><strong>Assinado no sistema</strong><span>{deliverySign.signerName} · {deliveryDriverSign.signerName}</span><small>{dateTime(deliverySign.signedAt)}</small></div> : <div><small>Entrega</small><strong>Sem assinatura</strong><span>Registre na ordem impressa</span></div>}{pickupSign && pickupDriverSign ? <div><small>Retirada / troca</small><strong>Assinado no sistema</strong><span>{pickupSign.signerName} · {pickupDriverSign.signerName}</span><small>{dateTime(pickupSign.signedAt)}</small></div> : <div><small>Retirada / troca</small><strong>Sem assinatura</strong><span>Opcional, obrigatória na cooperativa</span></div>}</div>{s.rentalSignatureRevisions.filter(item => item.rentalId === id).length > 0 && <small className="muted">{s.rentalSignatureRevisions.filter(item => item.rentalId === id).length} versão(ões) anterior(es) preservada(s), com autor, data e motivo.</small>}<Link className="text-button" href={`/ordem/${r.id}`}><Icon name="printer" size={14}/> Abrir ordem para assinar</Link></section>
 <section><h3>Datas previstas e realizadas</h3><div className="detail-grid three"><div><small>Entrega prevista</small><strong>{dateTime(r.deliveryAt)}</strong><small>Real: {dateTime(r.deliveredAt)}</small></div><div><small>Retirada prevista</small><strong>{pickupForecastLabel(r)}</strong><small>{pickupOverdue(r) ? 'Atrasada · ' : ''}Coleta: {dateTime(r.pickedUpAt)}</small></div><div><small>Retorno ao pátio</small><strong>{dateTime(r.returnedAt)}</strong><small>{s.settings.yardAddress || 'Pátio não configurado'}</small></div></div></section>
 <section><h3>Equipe e programação</h3>{jobs.map(j => <div className="job-detail" key={j.id}><div><strong>{LABEL[j.kind]}</strong><small>{s.drivers.find(d => d.id === j.driverId)?.name ?? 'Motorista de outra etapa'} · {s.trucks.find(t => t.id === j.truckId)?.code ?? 'Outro caminhão'}</small><small>{dateTime(j.scheduledAt)} · janela de {j.durationMinutes} min</small></div><Badge status={j.status}/>{!driver && j.status === 'SCHEDULED' && <Button small icon="calendar" onClick={() => openForm(scheduleForm(s, run, j))}>Reagendar</Button>}</div>)}{isOpenEndedPickup(r) && !pickupJob && <small className="muted">Retirada sob solicitação. Programe a coleta para atribuir equipe e horário, sem registrar movimentação antecipada.</small>}{driver && jobs.length < 2 && <small className="muted">Exibindo somente as etapas atribuídas a você.</small>}</section>
 {!driver && <section><div className="row split"><h3>Recebimentos</h3>{r.status !== 'CANCELLED' && (isMeasured(r) || paid < r.priceCents) && <Button small icon="plus" onClick={() => openForm(paymentForm(s, run, r))}>{isMeasured(r) ? 'Registrar valor diagnosticado' : 'Registrar pagamento'}</Button>}</div><div className="finance-summary"><div><small>{priceCaption(r)}</small><strong>{diagnosedLabel(r, paid)}</strong></div><div><small>Recebido</small><strong>{money(paid)}</strong></div><div><small>Saldo</small><strong>{balanceLabel(r, paid)}</strong></div></div>{isMeasured(r) && <div className="measurement-reasons"><small>Motivos do valor diagnosticado</small>{payments.some(p => !p.voidedAt && p.note.trim()) ? payments.filter(p => !p.voidedAt && p.note.trim()).map(p => <p className="pre-wrap" key={p.id}>{p.note}</p>) : <p className="muted">Ainda não informado. Ao registrar o valor, descreva volume, resíduo, diárias extras ou o critério usado.</p>}</div>}{payments.map(p => <div className={`payment-row ${p.voidedAt ? 'voided' : ''}`} key={p.id}><div><strong>{money(p.amountCents)} · {LABEL[p.method]}</strong><small>{dateTime(p.paidAt)} {p.voidedAt ? '· Estornado' : ''}</small>{p.note && <small className="pre-wrap">{isMeasured(r) ? `Motivos: ${p.note}` : p.note}</small>}</div>{!p.voidedAt && s.user.role === 'ADMIN' && <Button small onClick={() => openForm(voidForm(run, p))}>Estornar</Button>}</div>)}</section>}

...
export function RentalDetail({ id, onClose }: {
    ...
    const bin = s.containers.find(c => c.id === r.containerId), customer = s.customers.find(c => c.id === r.customerId), jobs = s.jobs.filter(j => j.rentalId === id), events = s.events.filter(e => e.rentalId === id), driver = s.user.role === 'DRIVER', paid = received(s, id);
 ...
 {!driver && r.status === 'RESERVED' && <Button variant="danger" icon="close" onClick={() => openForm(transitionForm(run, r, 'cancel'))}>Cancelar locação</Button>}</div></aside></div>;
}

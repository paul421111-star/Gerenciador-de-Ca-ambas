...
import { dateTime, money, LABEL, contractedLabel, balanceLabel, isOpenEndedPickup, customerPlaceLabel } from '../shared/format';
...
function SignatureBlock({ title, note, kind, responsibleLabel, driverLabel, defaultResponsible, defaultDriver, responsible, driver, canSave, canRedo, busy, notify, onSave }: {
    ...
    onSave: (kind: 'DELIVERY' | 'PICKUP', responsibleName: string, responsibleImage: string, driverName: string, driverImage: string) => Promise<void>;
}) {
    ...
    async function save() {
        ...
        try {
            await onSave(kind, left.name(), left.image(), right.name(), right.image());
            setRedo(false);
        }
        catch {
    ...
    return <section className="print-section"><h2>{title}{signed && <span className="signature-saved-mark">Assinado no sistema</span>}</h2>
        ...
        {canSave && !signed && <div className="signature-actions no-print"><Button variant="primary" disabled={busy} onClick={() => void save()}>Salvar assinaturas</Button><small className="muted">As assinaturas ficam gravadas nesta locação e aparecem na impressão.</small></div>}
        {canRedo && signed && <div className="signature-actions no-print"><Button disabled={busy} onClick={() => setRedo(true)}>Refazer assinaturas</Button></div>}
    </section>;
}
export function PrintOrder({ id }: {
    ...
    async function persist(kind: 'DELIVERY' | 'PICKUP', responsibleName: string, responsibleImage: string, driverName: string, driverImage: string) {
        try {
            await run('saveRentalSignatures', { rentalId: id, kind, responsibleName, responsibleImage, driverName, driverImage });
        }
        catch (e) {

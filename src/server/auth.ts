...
export function createAccount(db: DB, input: {
    name: string;
    email: string;
    password: string;
    role: Role;
    ...
    const id = randomUUID();
    db.prepare('INSERT INTO users(id,name,email,passwordHash,role,driverId,createdAt) VALUES(?,?,?,?,?,?,?)').run(id, input.name, input.email.trim().toLowerCase(), hashPassword(input.password), input.role, input.driverId, now);
    return id;
}
...
export function authenticate(db: DB, email: string, password: string, at = new Date()): {
    user: User;
    token: string;
    expiresAt: string;
} {
    ...
    const result = tx(db, () => {
        ...
        if (!stored || !stored.active || !valid) {
            ...
            db.prepare('INSERT INTO loginAttempts(key,count,updatedAt,blockedUntil) VALUES(?,?,?,?) ON CONFLICT(key) DO UPDATE SET count=excluded.count,updatedAt=excluded.updatedAt,blockedUntil=excluded.blockedUntil').run(key, count, now, count >= 5 ? new Date(at.getTime() + 15 * 60000).toISOString() : null);
            return { error: new AppError('E-mail ou senha incorretos.', 401) };
        }
        db.prepare('DELETE FROM loginAttempts WHERE key=?').run(key);
        ...
        db.prepare('INSERT INTO sessions(tokenHash,userId,expiresAt) VALUES(?,?,?)').run(sha256(token), stored.id, expiresAt);
...
export function sessionUser(db: DB, token: string | undefined, at = new Date()): User | null {
    if (!token || !/^[A-Za-z0-9_-]{43}$/.test(token))
        return null;
    const session = row<{
        userId: string;
    }>(db, 'SELECT userId FROM sessions WHERE tokenHash=? AND expiresAt>?', sha256(token), at.toISOString());
    return session ? row<User>(db, `SELECT ${USER_COLUMNS} FROM users WHERE id=? AND active=1`, session.userId) ?? null : null;
}
export function revokeSession(db: DB, token: string | undefined): void { if (token)

...
CREATE TABLE IF NOT EXISTS customerSites (
 ...
 latitude DOUBLE PRECISION, longitude DOUBLE PRECISION, locationPrecision TEXT NOT NULL DEFAULT 'PENDING' CHECK(locationPrecision IN ('CONFIRMED','APPROXIMATE','PENDING')),
 notes TEXT NOT NULL DEFAULT '', active INTEGER NOT NULL DEFAULT 1 CHECK(active IN (0,1)), createdAt TEXT NOT NULL,
 CHECK((latitude IS NULL) = (longitude IS NULL)),
 CHECK(latitude IS NULL OR (latitude BETWEEN -90 AND 90 AND longitude BETWEEN -180 AND 180)),
 CHECK((latitude IS NULL) = (locationPrecision = 'PENDING'))
);
CREATE UNIQUE INDEX IF NOT EXISTS idx_customer_sites_name ON customerSites (customerId, LOWER(name));
CREATE INDEX IF NOT EXISTS idx_customer_sites ON customerSites(customerId, active);
CREATE TABLE IF NOT EXISTS containers (
 id TEXT PRIMARY KEY, code TEXT NOT NULL UNIQUE, capacityM3 DOUBLE PRECISION CHECK(capacityM3 > 0),
...
CREATE TABLE IF NOT EXISTS rentals (
 ...
 CHECK(pickupAt IS NULL OR pickupAt > deliveryAt), CHECK((latitude IS NULL) = (longitude IS NULL)),
 CHECK(latitude IS NULL OR (latitude BETWEEN -90 AND 90 AND longitude BETWEEN -180 AND 180)),
 CHECK((latitude IS NULL) = (locationPrecision = 'PENDING')),
 CHECK((openEndedPickup = 1 AND pickupAt IS NULL) OR (openEndedPickup = 0 AND pickupAt IS NOT NULL) OR pickedUpAt IS NOT NULL)
);
...
CREATE TABLE IF NOT EXISTS rentalEvents (
 ...
 actorId TEXT NOT NULL REFERENCES users(id), occurredAt TEXT NOT NULL, latitude DOUBLE PRECISION, longitude DOUBLE PRECISION
);
...
CREATE TABLE IF NOT EXISTS rentalSignatureRevisions (
 id TEXT PRIMARY KEY, rentalId TEXT NOT NULL REFERENCES rentals(id), kind TEXT NOT NULL CHECK(kind IN ('DELIVERY','PICKUP')),
 role TEXT NOT NULL CHECK(role IN ('RESPONSIBLE','DRIVER')), signerName TEXT NOT NULL, image TEXT NOT NULL, signedAt TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_rental_signature_revisions ON rentalSignatureRevisions(rentalId, kind, replacedAt);
